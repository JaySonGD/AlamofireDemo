...
class SecondSmallController: UIViewController {
    ...
    private weak var controller: SecondFullScreenController?

    lazy var playView:SecondPlayView = {
        let playView = SecondPlayView(frame: CGRect(x: 0, y: NavbarHeight, width: kScreenWidth, height: 9 * kScreenWidth / 16.0))
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        playView.frame = CGRect(x: 0, y: NavbarHeight, width: kScreenWidth, height: 9 * kScreenWidth / 16.0)
...
extension SecondSmallController{
    ...
    private func present(to controller: SecondFullScreenController) {
        playView.state = .animating
